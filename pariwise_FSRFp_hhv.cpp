...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    //declare variables
    mxArray  *I_m,*I_m_rgb,*I_m_LD, *val, *gf_m, *IW_m, *NI_m, *Paramsptr_m;
    mwSize numdims;
...
//     double L = 4; 
    ...
    Paramsptr_m = mxDuplicateArray(prhs[3]);
        ...
        // for each pixel in the domain
            ...
            for(m=0;m<(Ns*Ns);m++) { //iterative all pixels in searching area
                ...
                if (idxq==idxp) {
                    ...
                    if (bProcess) { 
                        ...
                        for(int n=0; n<(Nr*Nr);n++) {
                            ...
                            double incremt = C2 * pow((b1*b2/(pow(b1,2)+pow(b2,2))), (2*L-1));
                            IWS = IWS*incremt; 
                            // nct = nct+1;
                        } 
                    }
                ...
                if (bProcess) {
                    bU = rand()%100+1;
                    // Pij
                    IW[m] = pow(IWS, 1/sig); // distance from the mth patch-sample to the referenced patch               
                    pij[m] = IWS;
